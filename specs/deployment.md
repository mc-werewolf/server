...
- 毎日(19:00 UTC = 04:00 JST)+ 手動実行(`workflow_dispatch`)で、prod環境のPostgreSQLを `pg_dump -Fc`(custom format。デフォルトでzlib圧縮される。実測で非圧縮の約1/19のサイズになることを確認済み)でダンプする。加えて、tag push による prod デプロイ前にも同じ保存先へ `werewolf-prod-before-deploy-*` としてダンプする
- 保存先は2箇所(冗長化):
  ...
  2. **Google Drive**(個人のDriveに作成したフォルダへアップロード。期限なし)
     ...
     - 必要なSecrets: `RCLONE_CONFIG`(`~/.config/rclone/rclone.conf` の中身全体。client_id/client_secret/refresh tokenを含む)、`GDRIVE_DESTINATION`(例: `minecraft/werewolf/server/backups`)
     - アップロード先は `gdrive:${GDRIVE_DESTINATION}`(マイドライブ配下、中間フォルダはrcloneが自動作成。フォルダIDでの指定はしない方針)。保存先をリポジトリごとに変える場合は、各リポジトリの `GDRIVE_DESTINATION` Secret だけを変更する
