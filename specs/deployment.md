...
- 毎日(19:00 UTC = 04:00 JST)で、prod環境のPostgreSQLを `pg_dump -Fc`(custom format。デフォルトでzlib圧縮される。実測で非圧縮の約1/19のサイズになることを確認済み)でダンプする。毎日の dump は prod 保存先配下の `daily/` に保存する。手動実行(`workflow_dispatch`)では dev/prod を選択できる。加えて、tag push による prod デプロイ前にも prod 保存先へ `werewolf-prod-before-deploy-*` としてダンプする
- 保存先は2箇所(冗長化):
  ...
  2. **Google Drive**(個人のDriveに作成したフォルダへアップロード)
     ...
     - アップロード先は毎日の prod backup なら `gdrive:${PROD_GDRIVE_DESTINATION}/daily`、prod deploy 前 backup なら `gdrive:${PROD_GDRIVE_DESTINATION}`、dev 手動 backup なら `gdrive:${DEV_GDRIVE_DESTINATION}`(マイドライブ配下、中間フォルダはrcloneが自動作成。フォルダIDでの指定はしない方針)
