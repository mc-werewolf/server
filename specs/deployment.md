...
4. **deploy-caddy ジョブ**(`needs: deploy`、実体は `kairo-js/github-workflows` の `caddy-snippet-deploy.yml` 呼び出し)
   ...
   - 次に `deploy-snippet` サブジョブが、werewolf-server の [`deploy/caddy/service.caddy`](../deploy/caddy/service.caddy) を checkout し、`envsubst` で `$BASIC_AUTH_DEV_USER` 等のplaceholderをSecretsの実値に置換した上で `/opt/proxy/conf.d/werewolf.caddy` へ配置し、`docker exec proxy-caddy caddy fmt --overwrite ...` → `docker exec proxy-caddy caddy validate ...` → `docker exec proxy-caddy caddy reload ...` を実行する
...
| `NEXT_DEPLOY_HOST` | app host移動準備用の新デプロイ先サーバーのホスト名/IP |
| `NEXT_DEPLOY_USER` | app host移動準備用の新デプロイ先サーバーのSSHユーザー |
| `NEXT_DEPLOY_SSH_KEY` | app host移動準備用の新デプロイ先サーバーへのSSH秘密鍵 |
...
| `DEV_BACKEND_UPSTREAM` | 任意。Caddyから見たdev backend upstream。未設定なら `werewolf-dev-backend:8000` |
| `DEV_FRONTEND_UPSTREAM` | 任意。Caddyから見たdev frontend upstream。未設定なら `werewolf-dev-frontend:3000` |
| `PROD_BACKEND_UPSTREAM` | 任意。Caddyから見たprod backend upstream。未設定なら `werewolf-prod-backend:8000` |
| `PROD_FRONTEND_UPSTREAM` | 任意。Caddyから見たprod frontend upstream。未設定なら `werewolf-prod-frontend:3000` |
| `DEV_BACKEND_PORT` | 任意。dev backendをappホストに公開するport指定。例: `127.0.0.1:18000` または `18000` |
| `DEV_FRONTEND_PORT` | 任意。dev frontendをappホストに公開するport指定。例: `127.0.0.1:13000` または `13000` |
| `PROD_BACKEND_PORT` | 任意。prod backendをappホストに公開するport指定。例: `10.0.0.10:8000` または `8000` |
| `PROD_FRONTEND_PORT` | 任意。prod frontendをappホストに公開するport指定。例: `10.0.0.10:3000` または `3000` |
| `NEXT_DEV_BACKEND_PORT` | 任意。`Prepare Next App Host` で新appホストにdev backendを公開するport指定 |
| `NEXT_DEV_FRONTEND_PORT` | 任意。`Prepare Next App Host` で新appホストにdev frontendを公開するport指定 |
| `NEXT_PROD_BACKEND_PORT` | 任意。`Prepare Next App Host` で新appホストにprod backendを公開するport指定 |
| `NEXT_PROD_FRONTEND_PORT` | 任意。`Prepare Next App Host` で新appホストにprod frontendを公開するport指定 |
...
- werewolf-server 側の役割
  ...
  - backend/frontendの接続先は `DEV_BACKEND_UPSTREAM` / `DEV_FRONTEND_UPSTREAM` / `PROD_BACKEND_UPSTREAM` / `PROD_FRONTEND_UPSTREAM` を `envsubst` で埋め込む。未設定時は従来通り `werewolf-dev-backend:8000` などの同一Docker network上のcontainer名にフォールバックする
  - `DEV_BACKEND_PORT` / `DEV_FRONTEND_PORT` / `PROD_BACKEND_PORT` / `PROD_FRONTEND_PORT` を設定すると、deploy時にCompose overrideを生成してbackend/frontendをappホストにpublishする。appを別サーバーへ切り出す場合は、Caddyから到達できるIP/portにbindし、対応する `*_UPSTREAM` を同じ到達先へ向ける
  - backend/frontendは `deploy/docker-compose.yml` 側で `container_name: ${APP_NAME}-${DEPLOY_ENV_NAME}-backend` / `-frontend` を明示的に付与しており、デフォルトupstreamはこの固定名(`werewolf-dev-backend` / `werewolf-dev-frontend` / `werewolf-prod-backend` / `werewolf-prod-frontend`)を参照する
    - (同一エイリアス名を持つ複数ネットワークに接続する場合、docker composeのデフォルトサービス名解決は曖昧になりうるため、明示的なcontainer_nameで一意にしている)
  - `deploy.yml` の `deploy-caddy` ジョブが `kairo-js/github-workflows` の `caddy-snippet-deploy.yml` へ snippetテンプレートのパスとBasic認証Secretsを渡し、`proxy-caddy` 本体の保証 → snippetレンダリング → `/opt/proxy/conf.d/werewolf.caddy` への配置 → `caddy validate` → `caddy reload` を行う(前述「GitHub Actions (CI/CD)」参照)
...
- ルーティング(snippetの中身)
  - `dev.mc-werewolf.com` → `/api/*` は `DEV_BACKEND_UPSTREAM`、それ以外は `DEV_FRONTEND_UPSTREAM`
  - `mc-werewolf.com` → `/admin*` は Basic認証つきで `PROD_FRONTEND_UPSTREAM`、`/api/*` は `PROD_BACKEND_UPSTREAM`、それ以外は `PROD_FRONTEND_UPSTREAM`
...
- 毎日(19:00 UTC = 04:00 JST)で、prod環境のPostgreSQLを `pg_dump -Fc`(custom format。デフォルトでzlib圧縮される。実測で非圧縮の約1/19のサイズになることを確認済み)でダンプする。毎日の dump は prod 保存先配下の `daily/` に保存し、30日を超えた `.dump` は削除する。手動実行(`workflow_dispatch`)では dev/prod を選択でき、各環境の `manual/` に保存する。加えて、tag push による prod デプロイ前にも prod 保存先配下の `versions/` へ `werewolf-prod-before-<tag>-*` としてダンプする
- 保存先は2箇所(冗長化):
...
- ワークフロー: [`.github/workflows/undeploy.yml`](../.github/workflows/undeploy.yml)
- 手動実行(`workflow_dispatch`)専用。`target` は `all` / `dev` / `prod` から選ぶ
- 誤実行防止のため、`confirm` に `undeploy werewolf` と完全一致で入力する必要がある
- `target=all` の場合は `werewolf.caddy` を削除し、`caddy validate` → `caddy reload` 後に dev/prod の compose stack を停止する。Caddy validate に失敗した場合は snippet を復元し、reload には進めない
- `target=dev` または `target=prod` の場合、Caddy snippet は dev/prod を1ファイルで管理しているため削除しない。該当環境の compose stack だけ停止する
- `remove-volumes=false` がデフォルト。`true` にすると `docker compose down -v` になり PostgreSQL volume も削除するため、移行完了後など明確に不要な場合だけ使う

## app host移動準備

- ワークフロー: [`.github/workflows/prepare-next-app-host.yml`](../.github/workflows/prepare-next-app-host.yml)
- 手動実行(`workflow_dispatch`)専用。`NEXT_DEPLOY_HOST` / `NEXT_DEPLOY_USER` / `NEXT_DEPLOY_SSH_KEY` を使い、新appホストへ同じDocker image tagをdeployする
- `restore-database=true` の場合、現在の `DEPLOY_HOST` から対象環境のPostgreSQL dumpを取得し、新appホストのPostgreSQLへ `pg_restore --clean --if-exists --no-owner` で復元する
- 復元後、新appホスト上で backend `/api/health`、`/api/health/db`、frontend `/` をコンテナ内部から確認する
- このworkflowはGitHub Secrets/Variablesを書き換えない。成功後、人間が GitHub UI で `DEPLOY_*` を `NEXT_DEPLOY_*` の値へ更新し、必要な `*_UPSTREAM` / `*_PORT` Variables を本番値へ切り替える
- 旧appホストの停止・削除は [`undeploy.yml`](../.github/workflows/undeploy.yml) で別途実行する
