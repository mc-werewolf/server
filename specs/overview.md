...
- 対象アドオン: `game-manager`, `vanillapack`, `additional-roles-1`, `kairo`, `kairo-database`
- 各アドオンの実体は GitHub Releases で配布される想定
- 本サーバーは「専用API」として、各アドオンの最新バージョン情報を提供する
  - bds-launcher はこのAPIに問い合わせ、ローカルのバージョンと比較して更新要否を判断する
  - 実バイナリのダウンロードを本サーバー経由にするか、GitHub Releasesへ直接誘導するかは未定
...
- アドオン実体の配信方法(本サーバー経由でバイナリ配信 or GitHub Releasesへのリダイレクトのみ)
